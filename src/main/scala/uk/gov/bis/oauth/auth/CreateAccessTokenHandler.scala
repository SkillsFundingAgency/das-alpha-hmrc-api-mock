...
import uk.gov.bis.oauth.data.{AuthRecord, AuthRecordOps, ClientOps, GatewayUser}
...
trait CreateAccessTokenHandler {
  ...
  private def buildAuthRecord(authInfo: AuthInfo[GatewayUser], privileged: Boolean): AuthRecord =
    data.AuthRecord(generateToken, Some(generateToken), None, authInfo.user.gatewayID, authInfo.scope, accessTokenExpiresInFourHours, timeSource.currentTimeMillis(), authInfo.clientId.get, Some(privileged))
