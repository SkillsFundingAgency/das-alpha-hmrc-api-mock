...
class EnrolmentsMongo @Inject()(val mongodb: ReactiveMongoApi, val timeSource: TimeSource)(implicit ec: ExecutionContext) extends MongoCollection[Enrolment] with EnrolmentOps {
  override def collectionName = "hmrc_enrolments"

  override def forGatewayID(gatewayID: String): Future[Seq[Enrolment]] = findMany("gatewayID" -> gatewayID)
}
