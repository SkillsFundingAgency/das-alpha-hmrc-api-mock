...
class LevyDeclarationMongo @Inject()(val mongodb: ReactiveMongoApi) extends LevyDeclarationOps {
  ...
  override def byEmpref(empref: String)(implicit ec: ExecutionContext): Future[Option[LevyDeclarationResponse]] = {
    val of = for {
    ...
    of.map {
      case Some(o) => o.validate[LevyDeclarationResponse].asOpt
      case _ => None
    }
  }
}
