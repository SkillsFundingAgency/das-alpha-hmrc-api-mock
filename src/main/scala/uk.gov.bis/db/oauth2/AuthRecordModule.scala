...
import uk.gov.bis.data.oauth2.{AuthRecord, AuthRecordOps}
import uk.gov.bis.db.SlickModule
import uk.gov.bis.playslicks.Enrolments
...
class AuthRecordDAO @Inject()(protected val authRecords: AuthRecordModule, gatewayIdSchemes: Enrolments)
  ...
  override def create(records: Seq[AuthRecord])(implicit ec: ExecutionContext): Future[Unit] = run(AuthRecords ++= records).map(_ => ())
